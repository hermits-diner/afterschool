...
- **수강신청 현황** – 전체 신청 내역 조회·검색·취소, **엑셀(CSV) 다운로드**, **현황 리포트 인쇄**(강좌별 요약 + 전체 내역)
...
- **내 수강신청** – 신청 강좌 목록·취소
- **내 시간표** – 주간 시간표 그리드, **시간표 인쇄**(그리드 + 강좌 목록 + 수강료 합계)
...
회원 일괄등록으로 생성되는 학생 아이디는 **연도(4) + 학번(4)** 8자리입니다.
학번은 학년(1)+반(1)+번호(2) — 예: 활성 세션이 2026학년도일 때 1학년 1반 1번 → `20261101`
...
|------|--------|----------|
| 관리자 | `admin` | `admin123` |
| 강사 | `teacher1` ~ `teacher6` | `teacher123` |
