...
**관리자 · 강사 · 학생** 세 가지 역할별 포털을 제공하며, 선착순 수강신청 · 실시간 정원 관리 ·
...
> ⚠️ **운영(서버리스) 환경에서는 데모 데이터가 생성되지 않고 `admin` 계정 1개만 만들어지며,
> 첫 로그인 시 비밀번호 변경이 강제됩니다.** 데모 데이터가 필요하면 `SEED_DEMO=true`를 설정하세요.
> 운영 시 `JWT_SECRET` 환경변수(64자 이상 무작위 문자열) 설정을 권장합니다 — 미설정 시 부팅 로그에 경고가 출력됩니다.
...
| POST | `/api/enrollments` | 수강신청 (선착순/충돌 검사) |
